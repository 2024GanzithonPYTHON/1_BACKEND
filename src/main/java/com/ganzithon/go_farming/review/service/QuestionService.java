...
import com.ganzithon.go_farming.review.dto.QuestionResponseDTO;
...
public class QuestionService {
    ...
    @Transactional
    public ResponseDTO<?> createQuestion(QuestionRequestDTO questionRequestDTO, Long reviewId, Long userId) {
    ...
    @Transactional
    public ResponseDTO<?> deleteQuestion(Long questionId, Long userId) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new CustomException(Exceptions.MEMBER_NOT_EXIST));
        Question question = questionRepository.findById(questionId)
                .orElseThrow(() -> new CustomException(Exceptions.QUESTION_NOT_EXIST));
        if (question.getUser() != user) {throw new CustomException(Exceptions.UNAUTHORIZED);}
